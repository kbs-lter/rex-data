...
subtractWater<-function(dataframe){
  ...
  water<- as.numeric(unlist(subset(dataframe,Well_C_Source == "Water",Delta)))
  #make a new blank column
  dataframe["Delta_blank"]<-dataframe$Delta - water
 return(dataframe)
}
...
biolog3 = select(biolog_subWater2, Delta_blank, Delta_SubHr0, Well_C_Source, Hour, Plot_ID, Sample_ID, Plate, Well, Biolog_column_ID) # remove all other columns 
View(biolog3)
