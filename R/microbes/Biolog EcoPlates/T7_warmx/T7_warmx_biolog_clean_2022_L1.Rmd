...
PROJECT: REX
Received help from a script made by Taylor Ulbrich
...
#Adding new column for negligable results (less than 0.06)
biolog1$Delta_new<- biolog1$Delta - biolog1$Delta_Water
biolog1$Delta_new[biolog1$Delta_new < 0.06] <- 0
...
biolog2 = select(biolog1, Delta_MinusWater, Delta, Delta_SubHr0, Delta_new, Well_C_Source, Hour, Plot_ID, Sample_ID, Plate, Well, Biolog_column_ID) # remove all other columns 
...
standardsamples3 <- select(standardsamples2, "Plot_ID", "Footprint_Location", "Rep", "Subplot_Descriptions",
                           "Biolog_column_ID", "SampleID_Hour", "Plate", "Hour", "Sample_ID", "Well", "Well_C_Source", "Delta_MinusWater", "Delta_new")

# change column "Delta_MinusWater" name to "Absorbence"
colnames(standardsamples3)[12] <- "Absorbence"

# adding carbon group column to data
# first make a list of all the carbon groups and the different carbon substrates that correspond to those groups
carbon_groups <- list(
...
# Create the new column with the broader group names based on the mapping above
standardsamples3$carbon_group <- unlist(sapply(standardsamples3$Well_C_Source, function(cs) {
...
standardsamples3 <- standardsamples3 %>%
  relocate(Absorbence, .after = last_col())

# save this as a completed, standardized data 
write.csv(standardsamples3, file.path(dir, "/microbes/Biolog EcoPlates/T7_warmx/L1/biolog_clean_updated_L1.csv"), row.names = FALSE)
...
##Checking to see if major differences between approaches to rounding absorbances to 0
```{r}
# histogram plots of absorbences using data frame standardsamples3
ggplot(standardsamples3, aes(x=Absorbence)) + 
  geom_histogram()

#histogram of new
ggplot(standardsamples3, aes(x=Delta_new)) + 
  geom_histogram()
```
